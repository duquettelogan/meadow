...
app.post('/api/v1/devices/register', async (req, res) => {
  ...
  if (!family_id || !platform || !device_token) {
    res
      .status(400)
      .json({ error: 'family_id, platform and device_token are required' });
    return;
  }
  try {
    const result = await db.query(
...
// ---------- Counters (aggregated, no URLs) ----------
app.get('/api/v1/children/:childId/blocks/today', async (req, res) => {
  const { childId } = req.params;
  try {
...
app.get('/api/v1/children/:childId/blocks/totals', async (req, res) => {
  const { childId } = req.params;
  try {
